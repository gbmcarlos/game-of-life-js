...
For example, to run a Artisan command, run `docker exec -it ecp /bin/sh -c "php artisan"`

## Admin user
The first time you will need to create and admin user. Run the `createAdminUser` artisan-enabled command (i.e.: `docker exec -it ecp /bin/sh -c "php artisan createAdminUser"`.
...
## Watch assets
To watch the assets (see the compiled changes instantly reflect after every change) run `docker exec -it -w /var/www ecp /bin/sh -c "node_modules/webpack/bin/webpack.js --hide-modules --config=node_modules/laravel-mix/setup/webpack.config.js --watch"`
